...
CURRENT_NODEJS_LTS_VERSION="$(wget -qO- https://nodejs.org/download/release/index.json | jq '.[] | select(.lts!=false) | .version' | grep -Eo '[0-9]+.[0-9]+.[0-9]+'| head -n 1)";
NODEJS_VERSION="$1";
[[ -z "$NODEJS_VERSION" ]] &&\
...
NODEJS_PATH="$(ls -d $NVP/$(ls $NVP | grep $NV | grep -v "\.tar\.gz$") 2>/dev/null)/bin";
NP=$NODEJS_PATH;
